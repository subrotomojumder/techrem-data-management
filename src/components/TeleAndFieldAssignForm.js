...
const TeleAndFieldAssignForm = ({ employee, dataStatus }) => {
    const [selectedData, setSelectedData] = useState([]);
    const [autoSelect, setAutoSelect] = useState(0);
    const [inputData, setInputData] = useState({ start: '', end: '' });
    const { data, isLoading, isError, error } = useGetAllDataQuery(`${employee.role === TELE_MARKETER ? "tele_marketer" : "fild_marketer"}_data=true&process=${employee.role === TELE_MARKETER ? "" : dataStatus?.value}`);
